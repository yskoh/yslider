var Slider = {
  ...
  interval: function (flag) {
    if (flag === 'start') {
      this.playSlider = window.setInterval(this.playSliderCallback.bind(Slider), this.intervalTime);
    } else if (flag === 'stop') {
  ...
  bindEvts: function () {
    this.elem.container.bind('click', this.handlers.clickHandlers.bind(Slider));
    if (this.autoPlay) {
      this.elem.contents.bind('mouseenter mouseleave', this.handlers.mouseHandlers.bind(Slider));
      this.interval('start');
    }
  },
  ...
  appendDots: function () {
    var $container = this.elem.container;
    var appendStr = '<div class="wrap-pager"><a href="" class="pager active" data-type="pager">1</a>';
  ...
  appendBtns: function () {
    if (this.bannerData.length > 2) {
      var appendStr = '<button type="button" class="arrow-btn prev" data-type="prev" ></button><button type="button" class="arrow-btn next" data-type="next"></button>';
  ...
  setOptions: function (settingsObj) {
    ...
    this.elem.container = $('div[data-type="sliderWrapper"]');
